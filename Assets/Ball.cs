...
public class Ball : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        float speedIncrement = 1.22f;  // 22% increment with each bounce!
        int maxIncrements = 4;
        ...
        switch (difficulty)  // check if difficulty is Hard or Nightmare - if so, adjust ball variables!
        {
            case "Hard":
                startingSpeed = 5f;
                speedIncrement = 1.44f;  // 44% increment with each bounce!
                maxIncrements = 7;
                break;
            case "Nightmare":
                startingSpeed = 5.55f;
                speedIncrement = 1.88f;  // 88% increment with each bounce!
                maxIncrements = 12;
        ...
        rb.velocity *= speedIncrement;  // increments ball speed by speedIncrement
    }
    ...
    void Update()
    {
        ...
        if (rb.position.x < -15 || rb.position.x > 15)
        {
            ...
            rb.velocity = new Vector2(xVelocity * startingSpeed, yVelocity * startingSpeed);  // resets velocity
